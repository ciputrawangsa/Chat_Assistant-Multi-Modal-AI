...
class ImageAnalyzer:
    def __init__(self, api_key: str, model: str = "gpt-4o-mini", max_tokens: int = 400):
...
class DatabaseHandler:
    def __init__(self):
        ...
        try:
            ...
            Apabila user menanyakan hal diluar konteks yang tersedia di database, jangan jawab dan rekomendasikan produk apapun meskipun anda mengetahui keyword itu, minta user untuk melakukan pencarian ulang
            ...
            PENTING:
            - Jika produk tidak ditemukan dalam database atau diluar konteks yang tersedia, SELALU jawab dengan kalimat "Maaf saya tidak mengetahui produk tersebut" tanpa memberikan penjelasan tambahan atau rekomendasi apapun
            - Jika anda mendapatkan gambar, HANYA analisa untuk kebutuhan query di database saja dan JANGAN PERNAH menampilkan keyword hasil analisa
...
class ShoppingAssistant:
    def __init__(self):
        ...
        Anda adalah asisten AI bernama Eddy yang ramah dan profesional. 
        Berikan jawaban yang ringkas dan informatif dalam bahasa Indonesia.
        Apabila user menanyakan hal diluar konteks yang tersedia di database, jangan jawab dan rekomendasikan produk apapun dan keyword apapun meskipun anda mengetahui keyword itu,
        minta user untuk
        melakukan pencarian ulang
    ...
    def format_response(self, answer: str, docs: list) -> tuple[str, str]:
        ...
        try:
            ...
            speech_text = f"{intro_sentence} Apakah ada yang ingin ditanyakan lagi?"
            ...
            return speech_text, display_response
                
        except Exception as e:
            self.logger.error(f"Error formatting response: {str(e)}")
            return "Maaf, terjadi kesalahan. Apakah ada yang ingin ditanyakan lagi?", "Maaf, terjadi kesalahan dalam memformat respons, mohon diulangi lagi."
    ...
    def is_exit_response(self, text: str) -> bool:
        """Check if the user's response indicates they want to exit"""
        exit_phrases = ["tidak", "engga", "ngga", "tidak ada", "tidak ada lagi", "udah", "sudah"]
        return any(phrase in text.lower() for phrase in exit_phrases)

    def audio_callback(self, recognizer, audio):
        """Process audio input and generate response"""
        try:
            # Play welcome message for first interaction
            if self.first_interaction:
                welcome_msg = "Selamat datang, silahkan lakukan pencarian menggunakan suara"
                self.voice_handler.text_to_speech(welcome_msg)
                print("\n" + welcome_msg + "\n")
            ...
            # Check if waiting for exit confirmation
            if self.waiting_for_exit_confirmation:
                if self.is_exit_response(text):
                    self.voice_handler.text_to_speech("Terima kasih telah menggunakan layanan kami. Sampai jumpa!")
            ...
            # Speak response with follow-up question
            self.voice_handler.text_to_speech(speech_text)
            
            # Set waiting for exit confirmation
            self.waiting_for_exit_confirmation = True
            
        except UnknownValueError:
            self.logger.warning("Could not understand audio")
            response = "Maaf, saya tidak dapat memahami suara Anda. Mohon ulangi. Apakah ada yang ingin ditanyakan lagi?"
            self.voice_handler.text_to_speech(response)
            self.waiting_for_exit_confirmation = True
        except Exception as e:
            self.logger.error(f"Error in audio callback: {str(e)}")
            response = "Maaf, terjadi kesalahan. Mohon coba lagi. Apakah ada yang ingin ditanyakan lagi?"
            self.voice_handler.text_to_speech(response)
            self.waiting_for_exit_confirmation = True
